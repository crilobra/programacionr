...
# Ventas por pais
prop_sales_by_country_by_year_month <- sales_all  %>% 
...
ggplot(data = prop_sales_by_country_by_year_month ) +
  geom_point(mapping = aes(x = country, y = percentage))

write.csv(prop_sales_by_country_by_year_month,'prop_sales_by_country_by_year_month.csv',row.names =TRUE)
